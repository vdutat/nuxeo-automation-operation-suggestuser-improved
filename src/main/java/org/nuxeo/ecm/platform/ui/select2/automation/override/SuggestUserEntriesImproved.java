 ...
 */
package org.nuxeo.ecm.platform.ui.select2.automation.override;
...
public class SuggestUserEntriesImproved {
    ...
    private static final Log log = LogFactory.getLog(SuggestUserEntriesImproved.class);
    ...
    public Blob run() throws ClientException {
        ...
        List<String> groupsInGroup;
        if (isGroupRestriction) {
        	groupsInGroup = userManager.getGroupsInGroup(groupRestriction);
        	groupsInGroup.add(groupRestriction);
        } else {
        	groupsInGroup = new ArrayList<String>();
        }
        try {
            ...
            if (!groupOnly) {
                ...
                for (DocumentModel user : userList) {
                    ...
                    if (isGroupRestriction) {
                        ...
                        if (groups != null && CollectionUtils.containsAny(groups, groupsInGroup)) {
                            result.add(obj);
                        }
                    } else {
                        result.add(obj);
